...
消息一般由两部分组成：**消息头**（或者说是标签 Label）和 **消息体**。消息体也可以称为 `payLoad `,消息体是不透明的，而消息头则由一系列的可选属性组成，这些属性包括 routing-key（路由键）、priority（相对于其他消息的优先权）、delivery-mode（指出该消息可能需要持久性存储）等。生产者把消息交由 `RabbitMQ `后，`RabbitMQ `会根据消息头把消息发送给感兴趣的 Consumer(消费者)。
...
**RabbitMQ 的 Exchange(交换器) 有4种类型，不同的类型对应着不同的路由策略**：**direct(默认)**，**fanout**, **topic**, 和 **headers**，不同类型的**Exchange**转发消息的策略有所区别。
...
以下内容建立在这样一个需求上： 我们将构建一个简单的日志记录系统。它将由多个程序组成——第一个程序将发出日志消息，余下程序将接收和打印日志消息。即现在有个一个生产者即一个日志来源程序，但同时有多个消费者即多个日志接收程序来实时打印或计算这些日志。如何让多个消费者都获取到消息？
...
对交换器来说，一方面，它接收来自生产者的消息，另一方面，它将这些消息推送到队列。交换器必须确切地知道如何处理它接收到的消息。它应该被追加到特定的队列中吗？它应该被追加到许多队列中吗？还是应该被丢弃。其规则由交换类型定义。
...
有几种可用的交换类型: `direct`, `topic`, `headers` 和 `fanout`。但在此我们先关注`fanout`。
...
// 将队列和交换器进行绑定，第三个参数表示路由键
channel.queueBind(queueName, "logs", "");
// 推送消息，此时logs交换器会负责将该消息传递到所绑定的队列
// 第一个参数是交换器名，第二个参数是路由键（此例中值被忽略），第三个参数是消息的属性包括消息类型
...
`direct`背后的路由算法很简单——将消息传递到其**绑定键**与消息的**路由键**这两者**完全匹配**的队列。
